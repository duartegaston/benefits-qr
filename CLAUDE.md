...
- **Local** (merchants): email/password OR Google OAuth → session cookie
- **Cliente** (customers): magic link via email (Resend) → session cookie

Both use the same `Session` table in the DB with a `userType` enum (`LOCAL | CLIENTE`). Cookie name: `session_token` (httpOnly, sameSite=lax).
...
- `getSessionFromCookies()` — for server components (uses `await cookies()`)
- `getSession(req)` — for API routes (reads from NextRequest)
...
### Key flows
- **Benefit claim**: Cliente opens `/beneficio/[id]` → enters email → `POST /api/reclamos` creates Reclamo + Session + sends magic link email
